 ...
 */
class BooksAdapter(val context: Context?, val books: ArrayList<Book>?) : BaseAdapter() {
    ...
    fun updateList(arr : ArrayList<Book>){
        books!!.addAll(arr)
    ...
    override fun getView(p0: Int, p1: View?, p2: ViewGroup?): View {
        ...
        viewHolder.titleTextView.text = books!![p0].title
        viewHolder.subtitleTextView.text = books[p0].subtitle

        viewHolder.pagesCount.text = "${books[p0].pagesCount} страниц"
        (viewHolder.coverImageView as NetworkImageView).setImageUrl(books[p0].imageLink, imageLoader)
